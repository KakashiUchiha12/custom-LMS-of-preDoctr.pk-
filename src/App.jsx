...
const CourseRouter = () => {
  ...
  if (subjectSlug.endsWith('-mcqs')) {
    return <TestDetail subjectId={subjectSlug.replace('-mcqs', '')} />;
  }
  
  if (subjectSlug.endsWith('-lectures')) {
    return <CourseDetail subjectId={subjectSlug.replace('-lectures', '')} />;
  }
  
  // Default fallbacks for specific manual routes
  if (subjectSlug === 'physics') return <TestDetail subjectId="physics" />;
  if (subjectSlug === 'past-papers-full-length') return <TestDetail subjectId="past-papers" />;
  if (subjectSlug === 'full-length-test-series') return <TestDetail subjectId="flt-series" />;
  if (subjectSlug === 'predoctr-test-series') return <TestDetail subjectId="predoctr-series" />;

  return <CourseDetail subjectId={subjectSlug} />;
};
    ...
    <div className="app">
      ...
      <main>
        <Routes>
          ...
          <Route path="/practice/saved/:folderName" element={
            <ProtectedRoute>
              <TestInterface />
            </ProtectedRoute>
          } />
